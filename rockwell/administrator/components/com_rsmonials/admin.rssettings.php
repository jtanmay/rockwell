...
function display() {
	...
	if($_REQUEST['limit'] > 0) {
		...
		$limit2 = 50;//$app->getCfg('list_limit');
	}
	if($_REQUEST['page'] > 0) {
		$pa = $_REQUEST['page'];
	}
...
?>
<div style="text-align:right;"><a href="index.php?option=<?php echo $_REQUEST['option']; ?>&view=<?php echo $_REQUEST['view']; ?>&action=editall&page=<?php echo $_REQUEST['page']; ?>&limit=<?php echo $_REQUEST['limit']; ?>" title="Edit All Item"><button type="button" class="btn hasTooltip js-stools-btn-clear">Edit All Settings</button></a></div>
<hr />
...
<?php } ?>
    ...
    <tbody>
        ...
        <tr class="row<?php echo ($cnt%2); ?>">
			...
			<?php
            if($item->param_name == 'blocked_ips') {
				echo '<spam style="color:black; font-weight:normal; font-size:85%;"><em>'.str_replace(',',', ',safeHTML($item->param_value)).'</em></span>';
			} else {
...
<fieldset class="adminform">
				...
				<input type="hidden" name="limit" id="limit" value="<?php echo $_REQUEST['limit']; ?>" />
					<table class="table table-striped" id="testisettingsList" style="position: relative;">
					<tr>
						<th class="key" nowrap="nowrap">Parameter Name:</td>
						<th>
					...
					<tr>
						<td class="key" nowrap="nowrap"><strong>Parameter Value:</strong></td>
						<td>
